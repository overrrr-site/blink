...
function ConcernSection({ concerns, dogId, onMutate }: ConcernSectionProps) {
  ...
  const handleAdd = useCallback(async () => {
    ...
    setSaving(true)
    try {
      await api.post(`/training-profiles/dogs/${dogId}/concerns`, {
        entry_date: newDate,
        note: newNote.trim(),
      })
      setNewNote('')
  ...
  }, [newNote, newDate, saving, dogId, onMutate])

  const handleUpdate = useCallback(async (entryId: number) => {
    if (!editNote.trim() || saving) return
    setSaving(true)
    try {
      await api.put(`/training-profiles/dogs/${dogId}/concerns/${entryId}`, {
        note: editNote.trim(),
      })
      setEditingId(null)
      onMutate()
  ...
  const handleDelete = useCallback(async (entryId: number) => {
    try {
      await api.delete(`/training-profiles/dogs/${dogId}/concerns/${entryId}`)
      setMenuOpenId(null)
      onMutate()
    } catch {
      // ignore
