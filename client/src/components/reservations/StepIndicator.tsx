type Step = 1 | 2 | 3

type StepIndicatorProps = {
  currentStep: Step
  canGoToStep2: boolean
  canGoToStep3: boolean
  onStepChange: (step: Step) => void
}

const STEP_LABELS: Record<Step, string> = {
  1: '日時',
  2: 'ワンちゃん',
  3: '確認',
}

function getStepButtonClass(stepNumber: Step, currentStep: Step): string {
  if (currentStep === stepNumber) {
    return 'bg-primary text-primary-foreground'
  }
  if (stepNumber < currentStep) {
    return 'bg-chart-2/10 text-chart-2'
  }
  return 'bg-muted text-muted-foreground'
}

function getStepCircleClass(stepNumber: Step, currentStep: Step): string {
  if (currentStep === stepNumber) {
    return 'bg-primary-foreground/20'
  }
  return 'bg-background'
}

export default function StepIndicator({
  currentStep,
  canGoToStep2,
  canGoToStep3,
  onStepChange,
}: StepIndicatorProps): JSX.Element {
  return (
    <div className="px-5 pt-4 mb-4">
      <div className="flex items-center gap-2">
        {([1, 2, 3] as const).map((stepNumber) => {
          const isClickable =
            stepNumber === 1 ||
            (stepNumber === 2 && canGoToStep2) ||
            (stepNumber === 3 && canGoToStep3)
          const isDisabled = stepNumber > currentStep && !isClickable

          return (
            <div key={stepNumber} className="flex items-center flex-1">
              <button
                type="button"
                onClick={() => {
                  if (isClickable || stepNumber <= currentStep) {
                    onStepChange(stepNumber)
                  }
                }}
                className={`flex-1 flex items-center gap-1.5 py-2 px-2 rounded-lg text-xs font-medium transition-colors ${getStepButtonClass(stepNumber, currentStep)}`}
                disabled={isDisabled}
              >
                <span
                  className={`size-5 rounded-full flex items-center justify-center text-[10px] font-bold ${getStepCircleClass(stepNumber, currentStep)}`}
                >
                  {stepNumber}
                </span>
                {STEP_LABELS[stepNumber]}
              </button>
              {stepNumber < 3 && <div className="w-2 h-px bg-border mx-1" />}
            </div>
          )
        })}
      </div>
    </div>
  )
}
