...
export default function HealthCheckForm({
  ...
  return (
    <div className="space-y-4">
      ...
      {/* 体重 */}
      <div>
        <label className="text-sm font-medium text-foreground mb-1 block">体重 (kg)</label>
        <input
          type="number"
          step="0.1"
          ...
          placeholder="例: 5.2"
          className="w-32 px-3 py-2 bg-background rounded-lg text-sm border-none focus:outline-none focus:ring-2 focus:ring-blue-200"
        />
      </div>
        ...
        {HEALTH_ITEMS.map(({ key, label, options }) => (
          <div key={key}>
            ...
            <select
              ...
              className="w-full px-3 py-2 bg-background rounded-lg text-sm border-none focus:outline-none focus:ring-2 focus:ring-blue-200 appearance-none"
            >
