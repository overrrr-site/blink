...
function getStatus(announcement: Announcement): AnnouncementStatus {
  const now = new Date()
...
function getStatusInfo(status: AnnouncementStatus): { label: string; color: string } {
  switch (status) {
    case 'published':
      return { label: '公開中', color: 'bg-chart-2/10 text-chart-2 border-chart-2/30' }
    case 'draft':
      return { label: '下書き', color: 'bg-chart-4/10 text-chart-4 border-chart-4/30' }
    case 'expired':
      return { label: '終了', color: 'bg-muted text-muted-foreground border-border' }
  }
}
...
function AnnouncementModal({ editingAnnouncement, onClose, onSaved }: AnnouncementModalProps): JSX.Element {
  ...
  const [formData, setFormData] = useState({
    ...
    is_important: editingAnnouncement?.is_important ?? false,
    published_at: toLocalDatetime(editingAnnouncement?.published_at ?? null),
    expires_at: toLocalDatetime(editingAnnouncement?.expires_at ?? null),
  })
...
function AnnouncementList(): JSX.Element {
  ...
  const statusCounts = { published: 0, draft: 0, expired: 0 }
  for (const a of announcements) {
    statusCounts[getStatus(a)]++
  }

  const filteredAnnouncements = statusFilter === 'all'
    ? announcements
    : announcements.filter((a) => getStatus(a) === statusFilter)
